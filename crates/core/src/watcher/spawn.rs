...
pub fn spawn_watcher(
    ...
    let mut debouncer = new_debouncer(Duration::from_millis(500), None, move |res: DebounceEventResult| {
        match res {
            Ok(events) => {
                for debounced in events {
                    ...
                    for raw in &debounced.event.paths {
                        ...
                        let kind = match &debounced.event.kind {
                            notify::event::EventKind::Create(_) => super::event::EventKind::Created,
                            notify::event::EventKind::Modify(_) => super::event::EventKind::Modified,
                            notify::event::EventKind::Remove(_) => super::event::EventKind::Removed,
                            _ => continue,
                        };
